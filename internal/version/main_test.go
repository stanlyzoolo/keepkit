...
// LoadCache/SaveCache, InstalledVersion) never write keepkit-*.log into the real
...
func TestMain(m *testing.M) {
	dir, err := os.MkdirTemp("", "keepkit-version-logs")
	if err != nil {
		panic(err)
	}
	restore := logx.SetDirForTesting(dir)
	brewDir, err := os.MkdirTemp("", "keepkit-version-brew")
	if err != nil {
		panic(err)
	}
	...
	cfgDir, err := os.MkdirTemp("", "keepkit-version-config")
	if err != nil {
		panic(err)
	}
...
func apiServer(t *testing.T, h apiHandlers) *apiHits {
	...
	// ~/.config/keepkit/token and doGH sends it to this local server.
