...
- **№2**: both commit handlers replace the `for i, mt := range m.meta` loop with `m.metaSelected = m.indexOfMeta(name)`; `updateTagsEdit` (mode.go:110-127) is the in-repo reference idiom.
- **№3**: remove `modeHelpSearch` from the `inputMode` enum and its dispatch; remove `m.helpSearch`, `m.helpMatches`, `m.helpMatchIdx`; remove the search branch in `helpContent()` (render.go:2414-2432) and the `renderStatusBar` branch (render.go:47-63 — note: this *is* the `N/M matches` surface; the `[3]` panel footer never carried one). The `case "/"` at model.go:1706 is **shared** — the brief/help branch returns early and falls through to the `modeSearch` entry, so the change is *adding a `focusTools` guard*, not deleting the case; otherwise `/` from `[2]`/`[3]` would open the tool-list search. Remove `findMatches`/`highlightMatch` (textutil.go) after a caller inventory. Drop `modeHelpSearch` from `flushPendingLaunch`'s funnel list and from the `[?]` overlay if listed. `/` stays bound only in `focusTools`.
...
- **№10 version-side**: new `fetchLatestTag(field)` — `GET /repos/{owner}/{repo}/tags` (first page), pick the max tag via `canonSemver`/`IsNewer` (fall back to the first entry when nothing canonicalizes; empty list → no answer). `getRepoData` calls it only when `fetchRelease` returned `errNoReleases`. Cache semantics, stated precisely:
  ...
  - Card consequence to pin in a test: a tag-derived `Latest` yields `↑` + an `enter` offer + no release date + "no release notes available." + **no clickable release heading** (render.go:1656-1668 already gates on `changelogURL != ""`).
- **№10 model-side**: today `errNoReleases` is already conclusive, so the per-visit cost is a goroutine + a `cache.json` read, **not** API quota — the settle is a hygiene fix, kept minimal. `m.repoStatus`-presence is **not** a usable marker (plan-review verified: a rate-limited pass carrying a stale card writes `"active"`/`"archived"` into `m.repoStatus` via model.go:998-1005, so presence would permanently suppress the retry). Therefore: a session-scoped `m.remoteAnswered map[string]bool`, written by the `remoteMsg` handler **only when `msg.err == nil`**, checked by `needsRemote`. The acceptance test must include the trap case: a rate-limited `remoteMsg` carrying a stale card (non-empty `repoStatus`, `ErrRateLimited` err) stays retryable.
...
- [ ] **add a `focusTools` guard to the shared `case "/"`** (model.go:1706) — the brief/help branch currently returns early and falls through to the `modeSearch` entry, so bare deletion would open the tool-list search from `[2]`/`[3]`
...
- [ ] update `flushPendingLaunch`'s funnel comment/list and the `[?]` overlay if it advertises the search
...
- [ ] add `mdFenceOpenRe` (captures the run), track the open marker, close via `rcFenceCloses(line, marker)`; remove `mdFenceRe`
...
- [ ] write failing struct-level test: `keepkitStyle(...).CodeBlock.Chroma` is a fresh pointer (plain pointer comparison against `styles.DarkStyleConfig.CodeBlock.Chroma` / the light twin), its `Background` background and `Text` color/background carry `t.Surface`/`t.Emphasis`, and every other token entry equals the stock one; `TestKeepkitStyleLeavesGlobalsUntouched` (readme_style_test.go:114) stays green
...
- [ ] write failing tests against `testAPIBase` fixtures: releases 404 + tags `[v1.2.0, v1.10.0]` → `Latest == "v1.10.0"` (semver max, not list order), `ReleaseMissing` stays true, `Body`/`HtmlUrl`/`PublishedAt` stay empty, the write is one `updateCacheEntry` mutate in `e := existing` style; releases 404 + empty tags → today's conclusive-but-blank outcome; tags fetch error (500/rate limit) → `conclusive` decision unchanged, entry identical to today's errNoReleases outcome (no veto of the `CheckedAt` stamp, no re-spend of the repo-info requests)
- [ ] implement `fetchLatestTag` + the `errNoReleases`-gated call in `getRepoData` (shared `force` core covers `RefreshRepoData`)
...
- [ ] pin the card consequence: tag-derived `Latest` → `↑` + `enter` offer + no release date + "no release notes available." + no clickable release heading (render.go:1656-1668 gates on `changelogURL != ""`)
...
- Modify: `internal/model/render_test.go` (the `needsRemote` table lives at :2200-2255)

- [ ] write failing tests: a tool whose `remoteMsg` arrived with `err == nil` and empty `Latest` (repo with neither releases nor tags) → `needsRemote` false (no re-dispatch per cursor visit); the trap case plan-review verified: a rate-limited `remoteMsg` **carrying a stale card** (non-empty `repoStatus`, `errors.Is(err, ErrRateLimited)`) → still retryable; rename/untrack clean the marker with the other per-name maps
- [ ] add session-scoped `m.remoteAnswered map[string]bool`, written by the `remoteMsg` handler **only when `msg.err == nil`**; `needsRemote` consults it; rename's stale-state cleanup deletes the old name
...
- [ ] re-read the CLAUDE.md sections touched by tasks 2, 5, 6, 10, 11, 12 as a whole — no contradictions left between them
